...
export class LkreTvSchedulerComponent implements OnInit {
  ...
  private getSeances(): Observable<Seance[]> {
  ...
  onToppingsChanged() {
    ...
    this.selectedSeancesString = this.seancesToString();
  }

  private seancesToString(): string {
    return this.selectedSeances.map(value => this.seanceToString(value)).join('\n');
  }

  private seanceToString(seance: Seance): string {
